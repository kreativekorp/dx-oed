/*
 * This is the dX firmware for the custom PCB with buttons.
 * If you have the Adafruit Trinket, please use dX_trinket instead.
 * If you have the custom PCB with piezo, please use dX_piezo instead.
 */
...
void setup() {
	...
	PORTB |=  0x1A; /* digitalWrite(1, HIGH); digitalWrite(3, HIGH); digitalWrite(4, HIGH); */
...
void loop() {
	...
	if (e) {
		...
		last_event = millis();
	} else if (settings_sleep_time && !in_menu() && ((millis() - last_event) / 1000) >= settings_sleep_time) {
		system_sleep();
	}
}
